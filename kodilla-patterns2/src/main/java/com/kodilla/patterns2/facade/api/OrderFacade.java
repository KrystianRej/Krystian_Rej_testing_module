    ...
    public void processOrder(OrderDto order, Long userId) throws OrderProcessingException{
        ...
        try {
            for (ItemDto orderItem: order.getItems()) {
                LOGGER.info("Assing item " + orderItem.getProductId() + ", " + orderItem.getQuantity() + " pcs");
            ...
            if (!shopService.doPayment(orderId)) {
                LOGGER.error(OrderProcessingException.ERR_PAYMENT_REJECTION);
                wasError = true;
                throw new OrderProcessingException(OrderProcessingException.ERR_PAYMENT_REJECTION);
            }
